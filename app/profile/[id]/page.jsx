          ...
          <section>
            {project.map((item, index) => (
                <Link href={`/project/${item.owner}_${index}`}>
                    <div key={index} className="flex flex-row w-1/2 gap-8 items-center m-auto py-8 px-10">
                            <div className="flex justify-between place-items-center w-full bg-white h-auto rounded-xl">
                                <span className="ms-5 text-2xl font-bold ">{item.title}</span> 
                                <Image className="rounded-xl" src={item.thumbnailurl} alt="project thumbnail" height={200} width={200} />
                            </div>
                    </div>
                </Link>
            ))}
